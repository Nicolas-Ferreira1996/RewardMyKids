...
const guideSchema = mongoose.Schema({
    ...
    childrenCollection: [{
        type: mongoose.Schema.Types.ObjectId, // mettre l'id des children dans un tableau du guide
        ref: 'children'  // Le nom du modèle référencé pour les enfants
    }]
});

guideSchema.pre("validate", async function (next) {
    try {//verifie si l'email est unique
        const existingguide = await this.constructor.findOne({email: this.email });
        if (existingguide) {
            this.invalidate("email", "Cet email est déjà enregistré."); //méthode invalidate qui permet de générer une erreur de validation
        }
        next();
    } catch (error) {
        next(error);
